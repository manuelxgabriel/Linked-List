...
void viewEmployees(Employee* pHead) {
	...
	while (pHead != nullptr) {
		...
		std::cout << "id: " << pHead->id << " name: " << pHead->name << std::endl;
		pHead = pHead->pNext;
...
Employee* createEmployee(const std::string& name) {
	...
	newEmployee->id = uniqueId++;
	newEmployee->name = name;
	newEmployee->pNext = nullptr;
...
NodeInfo getNodeInfo(Employee* pHead, int targetId) {
	NodeInfo result;
	result.pNode = nullptr;
	result.pParent = nullptr;

	if (pHead == nullptr) {
		return result;
	}

	// If it is the first node return that node
	if (pHead->id == targetId) {
		result.pNode = pHead;
		return result;
	}
	...
	while (current->pNext != nullptr) {
		if (current->pNext->id == targetId) {
			result.pParent = current->pNext;
			result.pNode = current->pNext->pNext;
			return result;
		}
